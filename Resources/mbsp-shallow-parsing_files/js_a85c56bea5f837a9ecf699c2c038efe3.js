...
;
// Finds <span> elements containing a bibliography title,
// and marks the parent <div> node as a bibliography node.
function onload_eventlistener(e) {
    if (document.body.className.indexOf("category") < 0 &&
        document.body.className.indexOf("taxonomy") < 0) {
        return;
    }
    $("span.biblio-title").parents("div.node").each(function(key, value) {
        value.className += " biblio-entry";
    });
}
try { window.addEventListener("load", onload_eventlistener, false); }
catch(e) { window.attachEvent("onload", onload_eventlistener); }

// Attempts to link pages deeper down the hierarchy to the main navigation,
// based on words in <body> classes and the text in navigation <a> elements.
var aliases = {
           'people': ['user', 'users'],
           'demos' : ['demo'],
    'publications' : ['archive', 'ctrs'],
         'software': ['pages-pattern', 'pages-mbsp']
};
function backtrack_navigation(e) {
    $('div#primary').find('a').each(function(key, value) {
        var v = value.innerHTML;
        if (document.body.className.indexOf("section-"+v) > 0 ||
            document.body.className.indexOf("node-type-"+v) > 0) {
            value.parentNode.className += " active";
            return false;
        }
        for(var i in aliases[v]) {
            if (document.body.className.indexOf("section-"+aliases[v][i]) > 0 ||
                document.body.className.indexOf("node-type-"+aliases[v][i]) > 0 ||
                document.body.className.indexOf("page-"+aliases[v][i]) > 0) {
                value.parentNode.className += " active";
                return false;
            }
        }
...
try { window.addEventListener("load", backtrack_navigation, false); }
catch(e) { window.attachEvent("onload", backtrack_navigation); }

// Opens a popup window for the given Quicktime movie.
// The movie URL must be relative.
function pop_movie(src, width, height) {
    src = "http://" + window.location.hostname + "/" + src;
    w = window.open(src, "", "width="+(width+50)+",height="+(height+50)+",scrollbars=0,resizable=1");
    w.onload = function() { 
        this.document.body.style.background = "#333";
        this.document.body.style.margin = 25;
        var e = this.document.getElementsByTagName("embed")[0];
        e.width = width;
        e.height = height+16; 
    }
    return false;
